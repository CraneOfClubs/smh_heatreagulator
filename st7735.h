...
{
public:
	static ST7745& instance()
	{
		static ST7745 s;
		return s;
	}

	void setMode(display_utils::SPIModes spi_mode);

	void init();
	void addrSet(uint16_t XS, uint16_t YS, uint16_t XE, uint16_t YE);

	void clear(uint16_t color);
	void orientation(display_utils::ScrOrientation_TypeDef orientation);
	...
	void putChar5x7(uint8_t scale, uint16_t X, uint16_t Y, uint8_t chr, uint16_t color, uint16_t bgcolor);
	void putChar7x11(uint16_t X, uint16_t Y, uint8_t chr, uint16_t color, uint16_t bgcolor);
	void putStr7x11(uint8_t X, uint8_t Y, char *str, uint16_t color, uint16_t bgcolor);
	void putStr5x7(uint8_t scale, uint8_t X, uint8_t Y, char *str, uint16_t color, uint16_t bgcolor);
private:

	display_utils::SPIModes spi_mode = display_utils::SPIModes::Second;

	//SPI configuration
	SPI_TypeDef* spi_port = SPI2;
	GPIO_TypeDef* spi_gpio_port = GPIOB;
	uint16_t spi_sck_pin = GPIO_Pin_13;
	uint16_t spi_mosi_pin = GPIO_Pin_15;

	uint32_t a0_port_clk = RCC_APB2Periph_GPIOA;
	GPIO_TypeDef* a0_port = GPIOA;
	uint16_t a0_pin = GPIO_Pin_11;

	uint32_t rst_port_clk = RCC_APB2Periph_GPIOA;
	GPIO_TypeDef* rst_port = GPIOA;
	uint16_t rst_pin = GPIO_Pin_12;

	uint32_t cs_port_clk = RCC_APB2Periph_GPIOB;
	GPIO_TypeDef* cs_port = GPIOB;
	uint16_t cs_pin = GPIO_Pin_5;

	uint16_t scr_width = 128;
	uint16_t scr_height = 160;
	...
	ST7745(); // конструктор недоступен
	~ST7745(); // и деструктор

	// необходимо также запретить копирование
	ST7745(ST7745 const&); // реализация не нужна
	ST7745& operator=(ST7745 const&); // и тут
};
...
namespace icons
{
	typedef struct
	{
		bool first_transparent;
		uint16_t palette[4]; // 1st - transparent;
		uint8_t pixel_array[128]; // 16 * 16 / 2
	} icon_16x16c8;
	...
	typedef struct
	{
		bool first_transparent;
		uint16_t palette[4]; // 1st - transparent;
		uint8_t pixel_array[512]; // 32 * 32 / 2
	} icon_32x32c8;
	...
	typedef struct
	{
		bool first_transparent;
		uint16_t palette[4]; // 1st - transparent;
		uint8_t pixel_array[64]; // 16 * 16 / 4
	} icon_16x16c4;
	...
	typedef struct
	{
		bool first_transparent;
		uint16_t palette[4]; // 1st - transparent;
		uint8_t pixel_array[256]; // 32 * 32 / 4
	} icon_32x32c4;

	typedef struct
	{
		bool first_transparent;
		uint16_t palette[2]; // 1st - transparent;
		uint8_t pixel_array[32]; // 16 * 16 / 8
	} icon_16x16c2;
	...
	typedef struct
	{
		bool first_transparent;
		uint16_t palette[2]; // 1st - transparent;
		uint8_t pixel_array[128]; // 32 * 32 / 8
	} icon_32x32c2;
}
